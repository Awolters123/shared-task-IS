...
def create_arg_parser():
    ...
    please see the help sections for a detailed description.'''
    ...
    parser.add_argument("-d2", "--data_file2", default="EXIST2021_merged.csv", type=str,
    ...
    parser.add_argument("-tf", "--transformer", default="GroNLP/hateBERT", type=str,
                        help="this argument takes the pretrained language model URL from HuggingFace, "
                             "default is HateBERT, please visit HuggingFace for full URL")
    # Parameter arguments
...
def train_transformer(lm, epoch, bs, lr, sl, X_train, Y_train, X_dev, Y_dev):
    """This function takes as input the train file, dev file, model name, and parameters.
...
def predict_dev(lm, sl, model, df_text, df_label):
    """This function takes as input the transformer model, maximum sequence length, Codalab dev set text and labels,
     and predicts and returns the classification report."""
...
def main():
    ...
    ori_train, ori_dev = train_test_split(ori, test_size=0.2, random_state=1234)
    ...
    if args.dev_text and args.dev_label:
        ...
        print(predict_dev(args.transformer, sl, HateBERT, df_dev_text, df_dev_label))
