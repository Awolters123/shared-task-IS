...
def create_arg_parser():
    ...
    parser.add_argument("-d2", "--data_file2", default="EXIST2021_merged.csv", type=str,
                        help="Extra dataset to train the model with, has to have compatible labels with "
                             "the first dataset")
    parser.add_argument("-t", "--test_file", default="dev_task_a_entries.csv", type=str,
                        help="Test file, which will be used to test the performance, "
                             "preferable unseen set without labels")
...
def predict(lm, sl, model, df_test):
    ...
    results.to_csv("EXAMPLE_SUBMISSION_dev_task_a.csv", index=False)
