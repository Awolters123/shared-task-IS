...
def create_arg_parser():
    ...
    model selection, and custom parameters, please see the help section
    for a detailed description."""
    parser = argparse.ArgumentParser()
    # Data input arguments
    parser.add_argument("-d1", "--data_file1", default="train_all_tasks.csv",
                        type=str, help="Dataset to train the model with, "
                                       "default is the SemEval 2022 "
                                       "sexism dataset")
    parser.add_argument("-d2", "--data_file2", type=str,
                        help="Extra dataset to augment "
    ...
    parser.add_argument("-d_label", "--dev_file_label", type=str,
                        help="Test file labels, "
                             "which will be used to evaluate the model")
    ...
    parser.add_argument("-tf", "--transformer", default="GroNLP/hateBERT",
                        type=str, help="this argument takes the pretrained "
                                       "language model link from HuggingFace, "
                                       "default is HateBERT")
...
def read_data(dataset, type):
    ...
    df.loc[df.label_category == '1. threats, plans to harm and incitement',
           'label_category'] = 0
    df.loc[df.label_category == '2. derogation', 'label_category'] = 1
    df.loc[df.label_category == '3. animosity', 'label_category'] = 2
    df.loc[df.label_category == '4. prejudiced discussions',
           'label_category'] = 3
...
def preprocess(text):
    """Removes hashtags and converts links to [URL] and usernames
    starting with @ to [USER], it also converts emojis
    to their textual form."""
    documents = []
...
def train_transformer(lm, epoch, bs, lr, sl, X_train, Y_train, X_dev, Y_dev):
    """This function takes as input the train file, dev file, transformer
    model name, and parameters. It trains the model with
    the specified parameters and returns the trained model."""
    ...
    model = TFAutoModelForSequenceClassification\
        .from_pretrained(lm, num_labels=4, from_pt=True)
    ...
    # Early stopping
    early_stopper = tensorflow.keras.callbacks\
...
def test_transformer(lm, epoch, bs, lr, sl, model, X_test, Y_test, ident):
    ...
    the model, parameters, and the test files, and predicts the labels for
    the test set and returns the accuracy score with a summarization
    of the model."""
    print(
    ...
    # the logits to softmax probabilities (multi-class)
...
def predict_test(lm, sl, model, df_test):
    ...
    # Getting predictions on the test set and converting
    # the logits to softmax probabilities
    Y_pred = model.predict(tokens_test)["logits"]
    prob = tensorflow.nn.softmax(Y_pred)
...
def main():
    ...
    df_train, df_dev = train_test_split(data, test_size=0.2,
                                        random_state=1234)
    ...
    if args.data_file2:
        ...
        print('Running HateBERT for task B with extra Exist2021 '
              'data in {} mode'.format(args.mode))
        HateBERT = train_transformer(args.transformer, ep, bs, lr, sl,
                                     X_train2, Y_train2, X_dev, Y_dev)
